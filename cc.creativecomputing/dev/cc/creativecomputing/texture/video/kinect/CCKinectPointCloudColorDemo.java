...
public class CCKinectPointCloudColorDemo extends CCApp {
	...
	@CCControl(name = "color x offset", min = -50, max = 50)
	private int _cXoffset = 0;
	
	@CCControl(name = "color y offset", min = -50, max = 50)
	...
	public void draw() {
		...
		for (int x = 0; x < CCKinect.DEVICE_WIDTH; x += skip) {
			for (int y = 0; y < CCKinect.DEVICE_HEIGHT; y += skip) {
				...
				CCVector2i myColorIndex = CCKinectUtil.worldToColor(myWorldCoords);
				
				CCColor myColor = _myKinect.colorData().getPixel(myColorIndex.x - _cXoffset, myColorIndex.y - _cYoffset);
				// Draw a point
				g.color(myColor);
	...
	public static void main(String args[]) {
		CCApplicationManager myManager = new CCApplicationManager(CCKinectPointCloudColorDemo.class);
