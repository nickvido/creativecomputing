...
/**
 * SVG stands for Scalable Vector Graphics, a portable graphics format. It is a vector format so it allows for infinite
 * resolution and relatively small file sizes. Most modern media software can view SVG files, including Adobe products,
 * Firefox, etc. Illustrator and Inkscape can edit SVG files.
 * <p>
 * We have no intention of turning this into a full-featured SVG library. The goal of this project is a basic shape
 * importer that is small enough to be included with applets, meaning that its download size should be in the
 * neighborhood of 25-30k. Starting with release 0149, this library has been incorporated into the core via the
 * loadShape() command, because vector shape data is just as important as the image data from loadImage().
 * <p>
 * For more sophisticated import/export, consider the <A HREF="http://xmlgraphics.apache.org/batik/">Batik</A> library
 * from the Apache Software Foundation. Future improvements to this library may focus on this properly supporting a
 * specific subset of SVG, for instance the simpler SVG profiles known as <A HREF="http://www.w3.org/TR/SVGMobile/">SVG
 * Tiny or Basic</A>, although we still would not support the interactivity options.
 * 
 * <p>
 * <hr noshade>
 * <p>
 ...
 * </PRE>
 * 
 * This code is based on the Candy library written by Michael Chang, which was later revised and expanded for use as a
 * Processing core library by Ben Fry. Thanks to Ricard Marxer Pinon for help with better Inkscape support in 0154.
 * 
 * <p>
 * <hr noshade>
 * <p>
 ...
 * </UL>
 * 
 * For those interested, the SVG specification can be found <A HREF="http://www.w3.org/TR/SVG">here</A>.
 */
public class PShapeSVG extends PShape {
	CCXMLElement element;
	...
	public PShapeSVG(CCXMLElement svg) {
		...
		String viewBoxStr = svg.attribute("viewBox");
		if (viewBoxStr != null) {
			String[] myValueStrings = CCStringUtil.splitTokens(viewBoxStr);
			width = Integer.parseInt(myValueStrings[2]);
			height = Integer.parseInt(myValueStrings[3]);
		}

		// TODO if viewbox is not same as width/height, then use it to scale
		// the original objects. for now, viewbox only used when width/height
		// are empty values (which by the spec means w/h of "100%"
		String unitWidth = svg.attribute("width");
		String unitHeight = svg.attribute("height");
		if (unitWidth != null) {
	...
	public PShapeSVG(PShapeSVG parent, CCXMLElement properties) {
		...
		if (parent == null) {
			...
			strokeWeight = 1;
			strokeCap = CCStrokeCap.SQUARE; // equivalent to BUTT in svg spec
		...
		if (name != null) {
			while (true) {
				String[] m = CCStringUtil.match(name, "_x([A-Za-z0-9]{2})_");
				if (m == null)
					break;
		...
		String displayStr = properties.attribute("display", "inline");
		visible = !displayStr.equals("none");

		String transformStr = properties.attribute("transform");
	...
	protected void parseChildren(CCXMLElement graphics) {
		children = new ArrayList<PShape>();

		for (CCXMLElement elem : graphics) {
	 ...
	 */
	protected PShape parseChild(CCXMLElement elem) {
		...
		if (name.equals("g")) {
			...
			CCLog.warn("Text and fonts in SVG files " + "are not currently supported, " + "convert text to outlines instead.");
			...
			CCLog.warn("Ignoring  <" + name + "> tag.");
		}
		return shape;
	}

	protected void parseLine() {
		primitive = CCShapePrimitive.LINE;
		family = PRIMITIVE;
		params = new float[] {
	...
	protected void parseEllipse(boolean circle) {
		primitive = CCShapePrimitive.ELLIPSE;
		family = PRIMITIVE;
		params = new float[4];
	...
	protected void parseRect() {
		primitive = CCShapePrimitive.RECT;
		family = PRIMITIVE;
		params = new float[] {
	...
	protected void parsePoly(boolean close) {
		...
		String pointsAttr = element.attribute("points");
		if (pointsAttr != null) {
			...
			for (int i = 0; i < vertexCount; i++) {
				...
				vertices[i][0] = Float.valueOf(pb[0]).floatValue();
				vertices[i][1] = Float.valueOf(pb[1]).floatValue();
			}
		}
	}
	...
	protected void parsePath() {
		...
		String pathData = element.attribute("d");
		if (pathData == null)
			return;
	...
	/**
	 * Parse the specified SVG matrix into a PMatrix2D. Note that PMatrix2D is rotated relative to the SVG definition,
	 * so parameters are rearranged here. More about the transformation matrices in <a
	 * href="http://www.w3.org/TR/SVG/coords.html#TransformAttribute">this section</a> of the SVG documentation.
	 * 
	...
	protected void parseColors(CCXMLElement properties) {
		if (properties.hasAttribute("opacity")) {
			String opacityText = properties.attribute("opacity");
			setOpacity(opacityText);
		}

		if (properties.hasAttribute("stroke")) {
			String strokeText = properties.attribute("stroke");
			setColor(strokeText, false);
		}

		if (properties.hasAttribute("stroke-opacity")) {
			String strokeOpacityText = properties.attribute("stroke-opacity");
		...
		if (properties.hasAttribute("stroke-width")) {
			...
			String lineweight = properties.attribute("stroke-width");
			setStrokeWeight(lineweight);
		}

		if (properties.hasAttribute("stroke-linejoin")) {
			String linejoin = properties.attribute("stroke-linejoin");
			setStrokeJoin(linejoin);
		}

		if (properties.hasAttribute("stroke-linecap")) {
			String linecap = properties.attribute("stroke-linecap");
		...
		if (properties.hasAttribute("fill")) {
			String fillText = properties.attribute("fill");
			setColor(fillText, true);
		}

		if (properties.hasAttribute("fill-opacity")) {
			String fillOpacityText = properties.attribute("fill-opacity");
			setFillOpacity(fillOpacityText);
		}

		if (properties.hasAttribute("style")) {
			String styleText = properties.attribute("style");
	...
	/**
	 * Parse a size that may have a suffix for its units. Ignoring cases where this could also be a percentage. The <A
	...
	static class Gradient extends PShapeSVG {
		...
		public Gradient(PShapeSVG parent, CCXMLElement properties) {
			super(parent, properties);

			CCXMLElement elements[] = properties.getChildren();
			offset = new float[elements.length];
			color = new int[elements.length];

			// <stop offset="0" style="stop-color:#967348"/>
			for (int i = 0; i < elements.length; i++) {
				CCXMLElement elem = elements[i];
	...
	class LinearGradient extends Gradient {
		...
		public LinearGradient(PShapeSVG parent, CCXMLElement properties) {
			super(parent, properties);
			...
			String transformStr = properties.attribute("gradientTransform");
	...
	class RadialGradient extends Gradient {
		...
		public RadialGradient(PShapeSVG parent, CCXMLElement properties) {
			super(parent, properties);
	...
	/**
	 * Get a particular element based on its SVG ID. When editing SVG by hand, this is the id="" tag on any SVG element.
	 * When editing from Illustrator, these IDs can be edited by expanding the layers palette. The names used in the
	 * layers palette, both for the layers or the shapes and groups beneath them can be used here.
	 * 
	 * <PRE>
