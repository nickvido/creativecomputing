...
//		frameRate(10);
		...
		for(int i = 0; i < 10;i++){
			_mySpline.addControlPoint(new CCVector3f(CCMath.random(-width/2, width/2), CCMath.random(-height/2, height/2)));
		}
...
//		g.blend(CCBlendMode.ADD);
		...
		_myShader.uniform1fv("splineLengths", _mySpline.segmentsLength());
		_myShader.uniform1f("splineLength", _mySpline.totalLength());
		_myShader.uniform1i("splinePointsLength", _mySpline.controlPoints().size());
