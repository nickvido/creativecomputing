...
public class CCDisplacementCatmulSubdivisionDemo extends CCApp {
	...
	private static class CCLinesShader{
		...
		private CCRenderTexture _myFilterTexture;
		...
		private CCRenderTexture createFilterTexture(final CCGraphics g, final int theTextureSize) {
			CCRenderTexture myFilterTexture = new CCRenderTexture(g, CCTextureTarget.TEXTURE_2D,theTextureSize, theTextureSize);
			myFilterTexture.wrap(CCTextureWrap.MIRRORED_REPEAT);
			myFilterTexture.textureFilter(CCTextureFilter.LINEAR);
			myFilterTexture.beginDraw();
			g.clear();
			g.beginOrtho2D();
...
//					g.color(1f);
		 ...
		 */
		public void start() {
			_myGraphics.texture(1,_myFilterTexture);
			_myShader.start();
