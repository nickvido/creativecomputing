...
public class CCGPUNearestGridPositionTargetForce extends CCGPUForce {
	...
	private CCShaderTexture _myCurrentTargetInfos;
	
	private CCShaderTexture _myCurrentParticleInfos;
	private CCShaderTexture _myDestinationParticleInfos;
	
	private CCTexture _myMaskTexture;
	...
	public void setSize(CCGraphics g, int theWidth, int theHeight) {
		...
		_myCurrentTargetInfos = new CCShaderTexture(32,4,_myGridWidth, _myGridHeight);
		
		_myCurrentParticleInfos = new CCShaderTexture(32,4,2,theWidth, theHeight);
		_myCurrentParticleInfos.clear();
		_myDestinationParticleInfos = new CCShaderTexture(32,4,2,theWidth, theHeight);
...
//		if(count < 100) {
		...
		_myNearestTargetShader.texture(_myTargetsTextureParameter, _myCurrentTargetInfos.id());
		_myNearestTargetShader.texture(_myParticleTargetInfoTextureParameter, _myCurrentParticleInfos.id());
		
		_myNearestTargetShader.texture(_myParticleInfoTextureParameter, _myParticles.positions().id(1));
		_myNearestTargetShader.texture(_myParticlePositionTextureParameter, _myParticles.positions().id(0));
		_myNearestTargetShader.texture(_myParticleVelocityTextureParameter, _myParticles.velocities().id(0));
		
		_myDestinationParticleInfos.draw();
		_myNearestTargetShader.end();

		CCShaderTexture myTemp = _myCurrentParticleInfos;
...
//		}
//		
		_myVelocityShader.texture(_myTargetPositionTextureParameter, _myCurrentParticleInfos.id());
		
		_myNearestTargetChangeShader.start();
		_myCurrentTargetInfos.beginDraw();
		_myGraphics.clearColor(0);
		_myGraphics.clear();
		_myNearestTargetChangeShader.texture(_myParticleTargetInfoTexture2Parameter, _myCurrentParticleInfos.id());
...
//		_myDestinationTargetInfos = myTemp;
	...
	public CCShaderTexture targetInfos() {
		return _myCurrentTargetInfos;
	}
	
	public CCShaderTexture particleTargetInfos() {
