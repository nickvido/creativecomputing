...
public class CCXMLIO{
	...
	public static class CCXMLParser<CCXMLTool extends CCAbstractXMLTool> implements Runnable{
		...
		private Reader handleStartTag(Reader page, StringBuffer alreadyParsed) throws Exception{
			...
			final Map<String,String> myAttributes = new HashMap<String,String>();
...
//					throw new RuntimeException("Illegal use of &. Use &amp; entity instead. Line:"+line);
	...
	public <XMLTool extends CCAbstractXMLTool> XMLTool loadElementNow(
		...
		CCXMLParser<XMLTool> myParser;
			
		if (documentUrl.startsWith("<?xml")){
			myParser = new CCXMLParser<XMLTool>(new StringReader(documentUrl),theXMLTool,null,theIgnoreLineFeed);
		}else{
			try{
				InputStream test = CCIOUtil.openStream(documentUrl);
				myParser = new CCXMLParser<XMLTool>(new BufferedReader(new InputStreamReader(test,"UTF-8")),theXMLTool,null,theIgnoreLineFeed);
			}catch (Exception e){
				return null;
			}
		}
		try{
			...
			throw new RuntimeException("XML was not able to read the given xml-file: " + documentUrl + " Please make sure that you load a file that contains valid xml.",e);
		}
	}
	...
	public static void saveXMLElement(final CCXMLElement theXMLElement, String theFilename){
		try{
			PrintStream myWriter = new PrintStream(CCIOUtil.createOutputStream(theFilename));
			myWriter.println(docStart);
			theXMLElement.print(myWriter, "\t");
			myWriter.flush();
			myWriter.close();
		}catch (Exception e){
			e.printStackTrace();
			System.out.println(CCIOUtil.dataPath(""));
