...
public class CCTextTexture extends CCPlacedTexture{
	...
	public void setText(final String theText){
		CCTextData myTextData = _myTextRenderer.createTexture(theText, _myFont, _myIsAntialised);
		
		_mySize = myTextData.size;
		_myWidth = myTextData.width;
		_myHeight = myTextData.height;
		
		_myRelativeAscent = (float)myTextData.fontMetrics.getAscent() / _mySize;
		_myRelativeDescent = (float)myTextData.fontMetrics.getDescent() / _mySize;
		_myRelativeWidth = _myWidth / _mySize;
		_myRelativeHeight = _myHeight / _mySize;
		_myRelativeOffset = myTextData.offset;
		
		_myRelativeOffset.scale(1f / _mySize);
		
		data(myTextData.textureData);
		myTextData.textureData.flush();
	}
