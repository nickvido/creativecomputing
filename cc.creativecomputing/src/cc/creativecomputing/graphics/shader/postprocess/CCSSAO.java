...
public class CCSSAO extends CCPostProcessEffect{
	...
	private CCShaderTexture	mFbo;
	private CCShaderTexture	mTempFbo;
	...
	public void initialize(int theWidth, int theHeight) {
		mFbo = new CCShaderTexture(theWidth, theHeight, CCTextureTarget.TEXTURE_2D);
		mTempFbo = new CCShaderTexture(theWidth, theHeight, CCTextureTarget.TEXTURE_2D);
	}
	
	@Override
	public void apply(CCGeometryBuffer theGeometryBuffer, CCGraphics g){

		g.texture(0, theGeometryBuffer.data(), CCGeometryBuffer.POSITIONS);
		g.texture(1, theGeometryBuffer.data(), CCGeometryBuffer.NORMALS);
