 ...
 *
 */
public class CCVector3f implements Cloneable, Serializable, CCIOctreeElement{
	
	protected float ALMOST_THRESHOLD = 0.001f;
	
	public static CCVector3f UP = new CCVector3f(0, 1, 0);
	public static CCVector3f RIGHT = new CCVector3f(1, 0, 0);
	public static CCVector3f OUT = new CCVector3f(0, 0, 1);
...
//	const Vector3 Vector3::Z = Vector3(0, 0, 1);
	...
	public boolean almost(float theX, float theY, float theZ) {
        if (CCMath.abs(x) - CCMath.abs(theX) < ALMOST_THRESHOLD && 
        	CCMath.abs(y) - CCMath.abs(theY) < ALMOST_THRESHOLD && 
        	CCMath.abs(z) - CCMath.abs(theZ) < ALMOST_THRESHOLD) {
            return true;
        } else {
            return false;
        }
