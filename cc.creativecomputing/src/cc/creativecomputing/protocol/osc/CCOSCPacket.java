...
public abstract class CCOSCPacket
{
	 ...
	 *			method.
	 *	 
	 *  @see	#getSize( CCOSCPacketCodec )
	 */
	public final int getSize(){
		return getSize( CCOSCPacketCodec.getDefaultCodec() );
	}
	 ...
	 */
	public final int getSize( CCOSCPacketCodec c ){
	...
	private static void printTextOn( PrintStream stream, CCOSCPacket p, int nestCount )
	{
		...
		if( p instanceof CCOSCMessage ) {
			...
			for( int i = 0; i < bndl.getPacketCount(); i++ ) {
				stream.println( "," );
				CCOSCPacket.printTextOn( stream, bndl.getPacket( i ), nestCount + 1 );
			}
			for( int i = 0; i < nestCount; i++ ) stream.print( "  " );
			stream.print( "]" );
		}
