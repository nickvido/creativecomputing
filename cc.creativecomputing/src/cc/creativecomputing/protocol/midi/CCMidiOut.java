...
public class CCMidiOut{
	 ...
	 */
	private void sendEvent(final CCMidiEvent theEvent){
	...
	private void sendEvent(final int theCommand, final int theData1, final int theData2){
		final CCMidiEvent event = new CCMidiEvent(theCommand, theData1, theData2);
		sendEvent(event);
	}
	 ...
	 */
	public void sendNote(final CCNote theNote){
		try{
			sendEvent(theNote);
	...
	public void sendNoteOn(final CCNote theNote){
		try{
			sendEvent(theNote);
		}catch (CCInvalidMidiDataException e){
			if (theNote.pitch() > 127 || theNote.pitch() < 0){
	...
	public void sendNoteOff(final CCNote theNote){
		try{
	 ...
	 *
	 */
	private static class CCCueNote extends CCMidiEvent{
		/**
		...
		CCCueNote(final CCMidiOut i_midiOut,final CCNote note, final long i_offTime){
	...
	private static class CCNoteBufferThread extends Thread{
		 ...
		 * @param theNote
		 */
		void addNote(final CCMidiOut theMidiOut, final CCNote theNote){
			_myNotes.add(new CCCueNote(theMidiOut, theNote, theNote.length()+_myNumberOfLoops));
		}
	}
}
