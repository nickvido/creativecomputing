...
public class CCDMX implements CCISerialListener {
	...
	private final static byte DMX_PRO_MESSAGE_START = (byte)0x7E;
    private final static byte DMX_PRO_MESSAGE_END = (byte)0xE7;
	 ...
	 */
	public CCDMX(CCAbstractApp theApp, String thePort, int theUniverseSize) {
        ...
        _myMessage[_myUniverseSize + 5] = DMX_PRO_MESSAGE_END;
	}
	 ...
	 * 
	 */
	public CCDMX(final CCAbstractApp theApp, final String thePort){
	...
	public void onSerialEvent(final CCSerial theSerial) {
		while (theSerial.available() > 0) {
			if (theSerial.read() == DMX_PRO_MESSAGE_START) {
				DMXMessage myMessage = new DMXMessage();
				myMessage.label = theSerial.read();
				myMessage.length = theSerial.read();
				myMessage.length += theSerial.read() << 8;

				myMessage.data = new byte[myMessage.length];

				for (int i = 0; i < myMessage.length; i++) {
					myMessage.data[i] = (byte) theSerial.read();
				}
