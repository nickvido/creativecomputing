...
public class CCModel {
	...
	public CCGroup group(final String theGroup){
		return _myGroupMap.get(theGroup.toLowerCase());
	}
	...
	public CCSegment segment(final String theGroup){
		return _myGroupMap.get(theGroup.toLowerCase()).segments().get(0);
	}
	...
	public void draw(CCGraphics g,final String theGroup){
		CCColorMaterialMode myColorMaterialMode = g.colorMaterial();
		g.colorMaterial(CCGraphics.OFF);
		CCGroup myGroup = _myGroupMap.get(theGroup.toLowerCase());
		if(myGroup != null){
			for(CCSegment mySegment:myGroup.segments()){
				mySegment.draw(g);
			}
